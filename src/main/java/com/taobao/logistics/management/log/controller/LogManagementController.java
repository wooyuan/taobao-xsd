package com.taobao.logistics.management.log.controller;

import com.taobao.logistics.management.log.service.LogCleanupJobService;
...
public class LogManagementController {
     ...
     * 判断是否为日志文件
     * 支持的日志文件格式：
     * 1. taobaoxsd.log（当前活跃日志文件）
     * 2. taobaoxsd.log.yyyy-MM-dd.gz（压缩的历史日志文件）
     * 3. taobaoxsd.log.yyyy-MM-dd（未压缩的历史日志文件）
     */
    private boolean isLogFile(String fileName) {
        ...
        // 匹配当前活跃日志文件
        if ("taobaoxsd.log".equals(fileName)) {
            return true;
        }
        ...
        // 匹配未压缩的历史日志文件（taobaoxsd.log.yyyy-MM-dd）
        if (fileName.startsWith("taobaoxsd.log.") && fileName.matches(".*\\d{4}-\\d{2}-\\d{2}$")) {
            return true;
        }
        
        // 为了向后兼容，保留对旧格式日志文件的支持
