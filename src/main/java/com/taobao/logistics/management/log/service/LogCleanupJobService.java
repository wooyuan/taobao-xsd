package com.taobao.logistics.management.log.service;
...
public class LogCleanupJobService {
     ...
     * 判断是否为日志文件（用于清理）
     * 匹配规则：
     * - taobaoxsd.log（当前活跃日志文件，不删除）
     * - taobaoxsd.log.yyyy-MM-dd.gz（压缩的历史日志文件，可删除）
     * - taobaoxsd.log.yyyy-MM-dd（未压缩的历史日志文件，可删除）
     * - taobao.log.yyyy-MM-dd（旧格式历史日志文件，可删除）
     * 
     * @param fileName 文件名
     * @return 是否为可删除的日志文件
     */
    private boolean isLogFile(String fileName) {
        ...
        // 为了向后兼容，匹配旧格式历史日志文件 taobao.log.yyyy-MM-dd
