...
public class RedirectController {
     ...
     * @return HTML格式的分配结果
     */
    @GetMapping(value = "/OsOrder", produces = MediaType.TEXT_HTML_VALUE + ";charset=utf-8")
    public String OsOrder(@RequestParam("orderId") Long orderId) {
        try {
            log.info("开始进行商城出库单智能订单分配，订单ID: {}", orderId);
            
            if (orderId == null || orderId <= 0) {
                return generateErrorHtml("订单ID无效", orderId);
            }
            
            String result = orderAllocationService.OssmartOrderAllocation(orderId);
            
            log.info("商城出库单订单分配完成，订单ID: {}", orderId);
            return result;
            
        } catch (Exception e) {
            log.error("商城出库单订单分配失败，订单ID: {}", orderId, e);
            return generateErrorHtml("系统异常: " + e.getMessage(), orderId);
        }
    }
